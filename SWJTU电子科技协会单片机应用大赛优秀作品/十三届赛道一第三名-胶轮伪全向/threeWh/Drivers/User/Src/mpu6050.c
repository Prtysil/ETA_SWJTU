...
{
    uint8_t buf[6],res;  
	res=MPU_Read_Len_Fast(MPU_GYRO_XOUTH_REG,6,buf);
	if(res==0)
	{
...
{
    uint8_t buf[6],res;  
	res=MPU_Read_Len_Fast(MPU_ACCEL_XOUTH_REG,6,buf);
	if(res==0)
	{
...
uint8_t MPU_Read_Len_Fast(uint8_t reg,uint8_t len,uint8_t *buf)
{ 
  extern I2C_HandleTypeDef hi2c1;
  HAL_I2C_Mem_Read(&hi2c1, MPU_READ, reg, I2C_MEMADD_SIZE_8BIT, buf, len, 0xfff);
...
uint8_t MPU_Read_Fast(uint8_t reg, uint8_t *buf){
  extern I2C_HandleTypeDef hi2c1;
  HAL_I2C_Mem_Read(&hi2c1, MPU_READ, reg, I2C_MEMADD_SIZE_8BIT, buf, 1, 0xfff);
